...
- 自动状态评估 - 类似自动记忆，每 6 回合调用当前 AI 汇总近期互动，调整情绪强度、精力和六维关系
...
├── auto_state.py      # 6 回合 AI 状态评估（情绪强度、精力、六维关系）
├── planner.py         # 反应计划生成器
...
CharacterRuntime.after_turn()
  ...
  ├── AutoState 每 6 回合调用 AI 更新情绪强度/六维关系
  ├── 保存运行时状态与关系状态
  └── 角色记忆服务抽取
  ↓
Pipeline AutoMemory
...
- **周期 AI 评估**: 每 6 回合汇总近期对话，让 AI 输出 mood、mood_intensity_delta、energy_delta 和六维关系增量；系统会限幅到安全范围后保存
