...
- **主动聊天心跳机制**：新增会话级主动聊天心跳，支持静默触发 AI 回复
  - Web 后台会话详情中可配置主动聊天开关与间隔（最小 1 分钟）
  - 开启时自动注册调度器，关闭时注销；全部关闭后停止调度以节省资源
  - 与生活模拟心跳、同步现实时间心跳共用统一调度入口，互不干扰
- **心跳管理器重构**：区分生活模拟与主动聊天会话两种场景
  - 配置 key 隔离：`{session_id}__proactive`、`sync_real_time`、`silent`
  - 避免不同场景互相覆盖间隔与退避计数

### 🎭 剧情系统

- **静默主动聊天场景**：AI 可在用户不在场时基于角色状态与昼夜节律自发产出主动消息
- **剧情选项生成优化**：改进选项生成逻辑，提升多样性与可玩性

### 🐳 Docker 部署

- **官方镜像发布**：提供多架构（amd64 / arm64）Docker 镜像，发布至 GitHub Container Registry (ghcr.io)
  - 新增 `Dockerfile`：基于 `python:3.11-slim`，内置 ffmpeg / git 等系统依赖
  - 新增 `.dockerignore`：避免将 `data/`、`.env`、缓存等打入镜像
  - 新增 GitHub Actions workflow `docker-publish.yml`：推送 master 或打 tag 自动构建并推送镜像
- **数据持久化**：镜像声明 `VOLUME /app/data`，部署时挂载即可保留会话 / 角色卡 / Web 配置
- **健康检查**：内置 `HEALTHCHECK`，定时探测 Web 端口存活状态

### 📖 文档

- 新增 [Docker 部署指南](./docker-deploy.md)
...
- **昼夜节律感知**：角色随现实时间（早/中/晚/深夜）呈现不同状态与对话风格，状态机引入"清醒/疲惫/困倦/熟睡"四阶段
- **情绪流动**：每轮情绪强度衰减/累积机制完善，6 轮 AI 汇总评估节奏更自然
- **跨会话时间线**：新增 `timeline` 记忆类别（路径 `characters/{char_id}/timeline.md`），记录跨会话的重要事件
  - 支持按 `[时间] [conv:会话ID] 内容` 结构化存储
  - 提示词注入按会话分桶（每个其他会话最多 2 条，全局最多 10 条），自动排除当前会话避免与 events 重复
  - 与 `important_event`（单会话）形成"近景/远景"互补：events 是当前会话剧情，timeline 是其他会话的近期生活
- **显式内心独白**：支持在回复中用 `（内心：...）` 表达内心活动
  - 前端实现可折叠灰字斜体渲染：默认隐藏展开按钮，点击展开/收起，不打断对话节奏
  - 流式渲染与多气泡渲染均兼容，XSS 安全过滤覆盖 `data-*` / `aria-*` 属性
  - PromptStack 中作为独立 `character.inner_monologue` 段注入
- **性格演化**：基础关系维度动态演化（好感/信任/熟悉/依赖/安全感/嫉妒六维），不依赖硬编码剧本
- **角色生活片段 (`life_sim`)**：新增静默心跳触发的"用户不在场"生活流
  - 文件路径按 `conversation_id` 严格隔离：`characters/{char_id}/life_sim/{conversation_id}.md`
  - 静默心跳调用 AI 生成 50-100 字的生活片段（思考/行动/情绪/状态），夜间仅生成睡眠相关内容
  - 最多保留 10 条/文件，提示词注入时取最新 5 条，header 明确说明"用户不在场"避免与对话事件混淆
  - 强化独处语境：明确禁用"用户/对话/互动"等措辞，杜绝幻觉产生"用户在场"内容

### 🪝 Heartbeat 系统重构

- **统一心跳入口**：所有心跳场景（主动聊天 / 同步现实时间 / 静默心跳）共用 `SessionHeartbeatManager` + `_run_session_heartbeat_execution`
  - 不同场景通过配置 key 隔离：`{session_id}__proactive`、`sync_real_time`、`silent`
  - 旧 `HeartbeatCore.process_user` 标记为 `[DEPRECATED]`，仅作回滚参考保留
- **二进制指数退避**：实际间隔 = `interval_minutes * 2^min(backoff_count, 4)`，上限 8 小时
  - 用户活跃时自动重置 `backoff_count = 0`，间隔迅速回到基线
  - Token 消耗显著降低：闲置越久间隔越长
- **Token 用途归类**：心跳产生的消耗归入 `PURPOSE_HEARTBEAT`
  - 静默心跳纯本地推断时不计 token，AI 触发生成时计费
  - 故障转移摘要新增 `consecutive_failures` 字段
- **会话详情"主动聊天"桥接**：将 `proactive_chat` 改造为会话级 heartbeat
  - PUT `/api/sessions/<id>` 更新 `proactive_chat` 时同步注册/注销心跳
  - 间隔由会话详情的 `interval_minutes` 控制，最小 1 分钟
  - 开启时启动调度器，全部关闭时停止调度
- **同步现实时间开启提示**：开启时弹窗确认 token 消耗，支持"不再提醒"勾选（写入 `localStorage`）

### 🧠 记忆系统

- **MemoryFS 记忆文件删除**：新增 `MemoryFS.delete(path)` 方法，Web 管理界面同步支持
- **结构化分类扩展**：v3.0.5 四类结构化体系新增 `timeline`（跨会话时间线）、`life_sim`（角色生活片段）两个新类别
  - 别名归一化：`timeline_event` / `life_event` / `life_sim_event` / `heartbeat_life` 全部归入对应类别
  - 路径分类判定顺序：`life_sim` → `events` → `plot`，避免误判
- **动态章节标题统一**：PromptStack 中的章节标题由 `## character.xxx` 改为 `[character.xxx]`，与 MemoryFS 风格统一
  - 兼容两种格式：剥离逻辑同时识别 `##` 和 `[]` 风格
  - 同 key 注入自动覆盖，避免重复段落

### 🤖 Agent 模式增强

- **公共分享优化**：Agent 模式会话支持公共分享，精简本子娘角色卡描述
- **进度卡片与待办卡片**：Agent 会话模式完整支持进度展示与待办管理
- **会话模式元数据透传**：将 `session_mode: "agent"` 同步到 `ctx.metadata`，AIPipeline 自动识别并跳过角色记忆注入

### 🖥️ Web UI 改进

- **会话切换与流式输出**：优化会话切换流畅度与流式输出体验
- **弹窗组件浅色主题**：为弹窗组件添加 light theme 适配样式
- **上下文摘要命令**：新增会话上下文压缩与查看命令，可查看完整摘要
- **会话列表稳定性**：修复 `/api/sessions` 返回格式不匹配导致会话列表闪消失的问题
- **AI 命令摘要长度**：移除上下文摘要的长度限制，显示完整摘要

### 🐛 Bug 修复

- **心跳每分钟强制触发**：修复静默心跳每分钟强制触发并错误发送消息的 bug
- **HTML 净化函数**：修复 `data-*` 和 `aria-*` 属性被错误剥离导致非流式渲染时内心独白 UI 状态丢失的问题
- **生活片段跨会话混乱**：修复静默心跳生活片段写入路径与读取路径不一致导致的跨会话数据污染（按 `conversation_id` 隔离）
- **夜间内容偏离**：修复静默心跳在夜间生成非休息状态内容的问题，强制仅生成睡眠相关状态

### 📖 文档

- 更新 `nbot/memory/fs.md`，补充 `timeline` 与 `life_sim` 两个新类别的路径规范、注入策略与归一化别名
...
- **会话 ID 跳转链接**：新增会话 ID 跳转链接组件，在会话管理、记忆管理等界面支持点击会话 ID 直接跳转到对应会话
- **移动端聊天界面优化**：优化移动端聊天界面布局并修复统计记录错误

### ⚙️ 核心功能

- 更新漫画专辑链接地址
- 抑制 apscheduler heartbeat 轮询的刷屏日志，减少日志噪音

### 📖 文档

- 在 README 中添加 NekoBot Android 客户端链接

### 🧹 工程优化

- 限制 pytest 仅在 `tests/` 目录下发现测试，避免意外扫描外部目录
- 移除已废弃的测试文件
