...
`;
                ...
                async loadOauthProviders() {
                    try {
                        ...
                        console.error('加载 OAuth providers 失败:', e);
                        this.showToast('加载 OAuth providers 失败: ' + (e.response?.data?.error || e.message), 'error');
                    }
                },

                async loadOauthAccounts() {
                    try {
                        const res = await api.get('/api/oauth/accounts');
                        if (res.data.success) {
                            this.oauthAccounts = res.data.accounts || [];
                        }
                    } catch (e) {
                        console.error('加载 OAuth 账号失败:', e);
                        this.showToast('加载 OAuth 账号失败: ' + (e.response?.data?.error || e.message), 'error');
                    }
                },

                getOauthProviderLogo(providerId) {
                    const logoMap = {
                        'openai-codex': '/static/svg/openai.svg',
                        'qwen-oauth': '/static/svg/qwen.svg',
                        'minimax-oauth': '/static/svg/minimax.svg',
                        'xai-oauth': '/static/svg/XAI.svg',
                        'opencode-zen': '/static/svg/opencode.svg',
                        'opencode-go': '/static/svg/opencode.svg',
                        'anthropic-oauth': '/static/svg/claude.svg',
                    };
                    return logoMap[providerId] || '/static/svg/openai.svg';
                },

                handleOauthLogoError(event, providerId) {
                    event.target.style.display = 'none';
                    const parent = event.target.parentElement;
                    if (parent) {
                        parent.textContent = (providerId || '?').charAt(0).toUpperCase();
                ...
                getOauthLoginButtonText(loginMode) {
                    const keyMap = {
                        'DEVICE_CODE': 'oauth_accounts.login',
                        'PKCE_CODE': 'oauth_accounts.login',
                        'QWEN_CREDENTIAL_IMPORT': 'oauth_accounts.import',
                        'API_KEY': 'oauth_accounts.input_key',
                    };
                    return this.$t(keyMap[loginMode] || 'oauth_accounts.login');
                },

                getOauthLoginIcon(loginMode) {
                    const iconMap = {
                        'DEVICE_CODE': 'fas fa-mobile-screen-button',
                        'PKCE_CODE': 'fas fa-right-to-bracket',
                        'QWEN_CREDENTIAL_IMPORT': 'fas fa-file-import',
                        'API_KEY': 'fas fa-key',
                    };
                    return iconMap[loginMode] || 'fas fa-right-to-bracket';
                },

                // 登录方式标签（provider 卡片底部的小标签）
                getOauthLoginModeLabel(loginMode) {
                    const keyMap = {
                        'DEVICE_CODE': 'oauth_accounts.mode.device_code',
                        'PKCE_CODE': 'oauth_accounts.mode.pkce',
                        'QWEN_CREDENTIAL_IMPORT': 'oauth_accounts.mode.credential_import',
                        'API_KEY': 'oauth_accounts.mode.api_key',
                    };
                    return keyMap[loginMode] ? this.$t(keyMap[loginMode]) : loginMode;
                },

                getOauthStatusText(status) {
                    const keyMap = {
                        'connected': 'oauth_accounts.status.connected',
                        'reauth_required': 'oauth_accounts.status.reauth_required',
                        'expired': 'oauth_accounts.status.expired',
                    };
                    return keyMap[status] ? this.$t(keyMap[status]) : status;
                },

                // 账号卡片左侧色条：按状态区分
                getOauthAccountToneClass(status) {
                    if (status === 'connected') return 'is-connected';
                    if (status === 'reauth_required') return 'is-warning';
                    if (status === 'expired') return 'is-danger';
                    return '';
                },

                getOauthDeviceStatusText(status) {
                    const keyMap = {
                        'pending': 'oauth_accounts.device.status.pending',
                        'connected': 'oauth_accounts.device.status.connected',
                        'failed': 'oauth_accounts.device.status.failed',
                    };
                    return keyMap[status] ? this.$t(keyMap[status]) : status;
                },

                selectAllOauthModels() {
                    this.oauthModelPickerSelected = [...new Set([
                        ...this.oauthModelPickerSelected,
                        ...this.filteredOauthModels,
                    ])];
                },

                clearOauthModelSelection() {
                    this.oauthModelPickerSelected = [];
                },

                async startOauthLogin(provider) {
                    this.oauthPendingProvider = provider;

                    if (provider.login_mode === 'QWEN_CREDENTIAL_IMPORT') {
                        this.oauthQwenRawJson = '';
                        this.showOauthQwenModal = true;
                        return;
                    }
                    if (provider.login_mode === 'API_KEY') {
                        this.oauthApiKeyInput = '';
                        this.showOauthApiKeyModal = true;
                        return;
                    }

                    // DEVICE_CODE 或 PKCE_CODE 都需要先调 /api/oauth/login
                    try {
                        this.isLoading = true;
                        const res = await api.post('/api/oauth/login', { provider: provider.id });
                        if (res.data.success) {
                            this.oauthLoginSession = res.data.session;
                            if (provider.login_mode === 'DEVICE_CODE') {
                                this.oauthDeviceStatus = 'pending';
                                this.oauthDeviceMessage = '';
                                this.showOauthDeviceModal = true;
                                this.startOauthPolling();
                            } else if (provider.login_mode === 'PKCE_CODE') {
                                this.oauthAnthropicCode = '';
                                this.showOauthAnthropicModal = true;
                            }
                        } else {
                            this.showToast('启动登录失败: ' + (res.data.error || '未知错误'), 'error');
                        }
                    } catch (e) {
                        console.error('启动 OAuth 登录失败:', e);
                        this.showToast('启动登录失败: ' + (e.response?.data?.error || e.message), 'error');
                    } finally {
                        this.isLoading = false;
                    }
                },
                ...
                startOauthPolling() {
                    ...
                    const intervalMs = (this.oauthLoginSession?.poll_interval_seconds || 5) * 1000;
                    this.oauthPollingTimer = setInterval(() => {
                        this.pollOauthLogin();
                    }, intervalMs);
                },
                ...
                async pollOauthLogin() {
                    if (!this.oauthLoginSession) return;

                    // 检查是否过期
                    if (Date.now() >= (this.oauthLoginSession.expires_at || 0)) {
                        this.stopOauthPolling();
                        this.oauthDeviceStatus = 'failed';
                        this.oauthDeviceMessage = '登录授权已过期，请重新开始';
                        return;
                    }

                    try {
                        const res = await api.post('/api/oauth/poll', { session: this.oauthLoginSession });
                        const result = res.data.result || res.data;
                        if (result.status === 'connected') {
                            this.stopOauthPolling();
                            this.oauthDeviceStatus = 'connected';
                            this.oauthDeviceMessage = '授权成功，正在加载模型列表...';

                            // 关闭设备码弹窗，打开模型选择
                            setTimeout(() => {
                                this.closeOauthDeviceModal();
                                this.loadOauthAccounts();
                                if (result.account) {
                                    this.openOauthModelPicker(result.account, result.models || []);
                                }
                            }, 1500);
                        } else if (result.status === 'failed') {
                            this.stopOauthPolling();
                            this.oauthDeviceStatus = 'failed';
                            this.oauthDeviceMessage = result.message || '授权失败';
                        }
                        // pending 状态继续轮询
                    } catch (e) {
                        console.error('轮询 OAuth 登录失败:', e);
                        // 网络错误时不停轮询，等下次重试
                    }
                },

                async submitOauthAnthropicCode() {
                    if (!this.oauthLoginSession || !this.oauthAnthropicCode.trim()) return;

                    try {
                        this.isLoading = true;
                        const res = await api.post('/api/oauth/anthropic-code', {
                            session: this.oauthLoginSession,
                            code_input: this.oauthAnthropicCode.trim(),
                        });
                        const result = res.data.result || res.data;
                        if (result.status === 'connected') {
                            this.showToast('Anthropic 授权成功', 'success');
                            this.closeOauthAnthropicModal();
                            this.loadOauthAccounts();
                            if (result.account) {
                                this.openOauthModelPicker(result.account, result.models || []);
                            }
                        } else {
                            this.showToast('授权失败: ' + (result.message || '未知错误'), 'error');
                        }
                    } catch (e) {
                        console.error('Anthropic 授权失败:', e);
                        this.showToast('授权失败: ' + (e.response?.data?.error || e.message), 'error');
                    } finally {
                        this.isLoading = false;
                    }
                },

                async importOauthQwenCredentials() {
                    if (!this.oauthQwenRawJson.trim()) return;

                    try {
                        this.isLoading = true;
                        const res = await api.post('/api/oauth/qwen-import', {
                            raw_json: this.oauthQwenRawJson.trim(),
                        });
                        const result = res.data.result || res.data;
                        if (result.status === 'connected') {
                            this.showToast('Qwen 凭证导入成功', 'success');
                            this.closeOauthQwenModal();
                            this.loadOauthAccounts();
                            if (result.account) {
                                this.openOauthModelPicker(result.account, result.models || []);
                            }
                        } else {
                            this.showToast('导入失败: ' + (result.message || '未知错误'), 'error');
                        }
                    } catch (e) {
                        console.error('Qwen 凭证导入失败:', e);
                ...
                async importOauthApiKey() {
                    if (!this.oauthPendingProvider || !this.oauthApiKeyInput.trim()) return;

                    try {
                        this.isLoading = true;
                        const res = await api.post('/api/oauth/api-key', {
                            provider: this.oauthPendingProvider.id,
                            api_key: this.oauthApiKeyInput.trim(),
                        });
                        const result = res.data.result || res.data;
                        if (result.status === 'connected') {
                            this.showToast(this.oauthPendingProvider.title + ' 登录成功', 'success');
                            this.closeOauthApiKeyModal();
                            this.loadOauthAccounts();
                            if (result.account) {
                                this.openOauthModelPicker(result.account, result.models || []);
                            }
                        ...
                        console.error('OAuth API Key 登录失败:', e);
                        this.showToast('登录失败: ' + (e.response?.data?.error || e.message), 'error');
                    } finally {
                        this.isLoading = false;
                    }
                },

                async openOauthModelPicker(account, presetModels) {
                    this.oauthModelPickerAccount = account;
                    this.oauthModelPickerAvailable = presetModels || [];
                    this.oauthModelPickerSelected = [];
                    this.oauthModelPickerLoading = false;
                    this.oauthModelSearch = '';
                    this.showOauthModelPickerModal = true;

                    // 加载已缓存模型和已选模型
                    try {
                        const res = await api.get(`/api/oauth/accounts/${account.id}/models`);
                        if (res.data.success) {
                            if (res.data.available?.length > 0) {
                                this.oauthModelPickerAvailable = res.data.available;
                            }
                            this.oauthModelPickerSelected = res.data.selected || [];
                        }
                    } catch (e) {
                        console.error('加载 OAuth 模型列表失败:', e);
                        this.showToast('加载模型列表失败: ' + (e.response?.data?.error || e.message), 'error');
                    }
                },

                closeOauthModelPicker() {
                    this.showOauthModelPickerModal = false;
                    this.oauthModelPickerAccount = null;
                    this.oauthModelPickerAvailable = [];
                    this.oauthModelPickerSelected = [];
                },

                async refreshOauthAccountModels() {
                    if (!this.oauthModelPickerAccount) return;

                    try {
                        this.oauthModelPickerLoading = true;
                        const res = await api.post(`/api/oauth/accounts/${this.oauthModelPickerAccount.id}/models`, {
                            refresh: true,
                        });
                        if (res.data.success) {
                            this.oauthModelPickerAvailable = res.data.available || [];
                        ...
                        this.showToast('刷新失败: ' + (e.response?.data?.error || e.message), 'error');
                    } finally {
                        this.oauthModelPickerLoading = false;
                    }
                },

                async saveOauthAccountModels() {
                    if (!this.oauthModelPickerAccount) return;

                    try {
                        this.oauthModelPickerLoading = true;
                        const res = await api.put(`/api/oauth/accounts/${this.oauthModelPickerAccount.id}/models`, {
                            selected: this.oauthModelPickerSelected,
                        });
                        if (res.data.success) {
                            this.showToast('模型保存成功', 'success');
                            this.closeOauthModelPicker();
                            this.loadOauthAccounts();
                            // 同时刷新 AI 模型列表，因为 OAuth 模型会写入 ai_models
                            if (typeof this.loadAIModels === 'function') {
                                this.loadAIModels();
                            }
                        } else {
                            this.showToast('保存失败: ' + (res.data.error || '未知错误'), 'error');
                        }
                    } catch (e) {
                        console.error('保存 OAuth 模型失败:', e);
                        this.showToast('保存失败: ' + (e.response?.data?.error || e.message), 'error');
                    } finally {
                        this.oauthModelPickerLoading = false;
                    }
                ...
                async deleteOauthAccount(account) {
                    this.showConfirm({
                        title: this.$t('oauth_accounts.delete_title'),
                        message: this.$t('oauth_accounts.delete_confirm', { label: account.label }),
                        impact: this.$t('oauth_accounts.delete_impact'),
                        confirmText: this.$t('oauth_accounts.delete'),
                        danger: true,
                        onConfirm: async () => {
                            try {
                                this.isLoading = true;
                                const res = await api.delete(`/api/oauth/accounts/${account.id}`);
                                if (res.data.success) {
                                    ...
                                    this.showToast('删除失败: ' + (res.data.error || '未知错误'), 'error');
                                }
                            } catch (e) {
                                console.error('删除 OAuth 账号失败:', e);
                                this.showToast('删除失败: ' + (e.response?.data?.error || e.message), 'error');
                            } finally {
                                this.isLoading = false;
                            }
                ...
                closeOauthDeviceModal() {
                    this.showOauthDeviceModal = false;
                    this.stopOauthPolling();
                    this.oauthLoginSession = null;
                    this.oauthPendingProvider = null;
                    this.oauthDeviceStatus = 'pending';
                    this.oauthDeviceMessage = '';
                },

                closeOauthAnthropicModal() {
                    this.showOauthAnthropicModal = false;
                    this.oauthLoginSession = null;
                    this.oauthPendingProvider = null;
                    this.oauthAnthropicCode = '';
                },
                ...
                closeOauthQwenModal() {
                    this.showOauthQwenModal = false;
                    this.oauthQwenRawJson = '';
                    this.oauthPendingProvider = null;
                },

                closeOauthApiKeyModal() {
                    this.showOauthApiKeyModal = false;
                    this.oauthApiKeyInput = '';
                    this.oauthPendingProvider = null;
                },

                copyOauthUserCode() {
                    if (!this.oauthLoginSession?.user_code) return;
                    navigator.clipboard.writeText(this.oauthLoginSession.user_code).then(() => {
                        this.showToast('设备码已复制', 'success');
                    }).catch(() => {
                        this.showToast('复制失败，请手动选择复制', 'error');
                    });
                },

                openOauthVerificationUrl() {
                    if (this.oauthLoginSession?.verification_url) {
                        window.open(this.oauthLoginSession.verification_url, '_blank', 'noopener,noreferrer');
                    }
                },
    ...
    editHook(hook) {
        ...
        this.newHook = {
            ...
            condition_logic: hook.condition_logic || 'and',
    ...
    async createHook() {
        ...
        const payload = {
            ...
            condition_logic: this.newHook.condition_logic || 'and',
